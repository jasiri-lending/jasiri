...
const ReportWrapper = ({ component: Component, permission, userRole, ...rest }) => {
  ...
  if (permission && !hasPermission(permission)) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] p-8 text-center bg-white rounded-xl shadow-sm border border-gray-100 mx-auto max-w-2xl mt-12">
        <div className="w-16 h-16 bg-red-50 rounded-full flex items-center justify-center mb-4">
          <Lock className="w-8 h-8 text-red-500" />
        </div>
        <h3 className="text-xl font-bold text-gray-800 mb-2">Access Denied</h3>
        <p className="text-gray-600 mb-6 max-w-md">
          You do not have the required permission (<strong>{permission}</strong>) to view this report.
  ...
  return <Component userRole={userRole} {...rest} />;
};
    ...
    <PermissionProvider>
      <ToastProvider>
        <div className="flex h-screen bg-gray-100 overflow-hidden">
          {showShell && renderSidebar()}

          <div className="flex flex-col flex-1 min-w-0">
            {showShell && renderHeader()}
                ...
                <Routes>
                  ...
                  {isSharedRole && (
                    <>
                      ...
                      />
                      <Route
                        path="/registry/customer-edits/review/:requestId/:requestType"
                        element={
                          <ProtectedRoute>
                      ...
                      />
                      <Route
                        path="/reports/ro-cumulative"
                        element={
                          <ProtectedRoute>
                            <ReportsLayout>
                              <ROCumulativePerformanceReport userRole={role} />
                            </ReportsLayout>
                          </ProtectedRoute>
                        }
                      />
