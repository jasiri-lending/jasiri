...
const RejectedLoans = () => {
  ...
  // Get rejected by info
  const getRejectedBy = (loan) => {
    if (loan.bm_decision === 'rejected') {
      return {
        text: "Branch Manager",
        color: "bg-red-100 text-red-800"
      };
    } else if (loan.rm_decision === 'rejected') {
      return {
  ...
  // Get rejection date
  const getRejectionDate = (loan) => {
    if (loan.bm_reviewed_at && loan.bm_decision === 'rejected') {
      return loan.bm_reviewed_at;
    } else if (loan.rm_reviewed_at && loan.rm_decision === 'rejected') {
      return loan.rm_reviewed_at;
    }
    return null;
  };

  // Get rejection comment
  const getRejectionComment = (loan) => {
  ...
  if (authLoading || loading) {
    return (
      <div className="h-full bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 p-6 min-h-screen flex items-center justify-center">
        <Spinner text="Loading rejected loans..." />
      </div>
    );
  }
  ...
  return (
    <div className="h-full bg-gradient-to-br from-slate-50 via-blue-50 to-cyan-50 text-gray-800 border-r border-gray-200 transition-all duration-300 p-6 min-h-screen font-sans">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-xs text-white px-3 py-1.5 rounded-lg border border-gray-200 shadow-sm" style={{ backgroundColor: "#dc2626" }}>
          <span className="font-medium text-white">{rejectedLoans.length}</span> rejected loan{rejectedLoans.length !== 1 ? 's' : ''}
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b" style={{ backgroundColor: '#E7F0FA' }}>
                ...
                <th className="px-4 py-3 text-center text-xs tracking-wider whitespace-nowrap" style={{ color: '#0D2440' }}>Rejected By</th>
                <th className="px-4 py-3 text-center text-xs tracking-wider whitespace-nowrap" style={{ color: '#0D2440' }}>Rejected Date</th>
            ...
            <tbody>
              {currentLoans.map((loan, index) => {
                ...
                const rejectedBy = getRejectedBy(loan);
                const rejectionDate = getRejectionDate(loan);

                return (
                  <tr key={loan.id} className={`border-b transition-colors hover:bg-red-50 ${index % 2 === 0 ? '' : 'bg-red-50/50'}`}>
                    ...
                    <td className="px-4 py-3 text-sm text-center whitespace-nowrap">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${rejectedBy.color}`}>
                        {rejectedBy.text}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-sm text-center whitespace-nowrap" style={{ color: '#0D2440' }}>
                      <div className="flex items-center justify-center gap-1">
                        ...
                        {rejectionDate ? formatDate(rejectionDate) : "N/A"}
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-center whitespace-nowrap">
                      <div className="flex items-center justify-center gap-1.5">
                        <button
                          ...
                          className="p-2 rounded-lg bg-gradient-to-r from-red-50 to-red-100 border border-red-200 text-red-600 hover:from-red-100 hover:to-red-200 hover:text-red-700 hover:border-red-300 transition-all duration-200 shadow-sm hover:shadow"
      ...
      {selectedLoan && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-200">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">Rejection Details</h3>
              <button
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <div className="text-lg font-semibold text-emerald-600">
                    Ksh {selectedLoan.scored_amount?.toLocaleString()}
                  </div>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-500">Rejection Date</label>
                  <div className="text-lg font-semibold text-gray-900">
                    {formatDate(getRejectionDate(selectedLoan))}
                  </div>
                </div>
                {showBranchColumn && (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className={`text-lg font-semibold ${selectedLoan.bm_decision === 'rejected' ? 'text-red-600' : 'text-orange-600'
                    }`}>
                    {getRejectedBy(selectedLoan).text}
                  </div>
                </div>
                <div>
              ...
              <div>
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                  <p className="text-gray-800">
                    {getRejectionComment(selectedLoan)}
                  </p>
                </div>
              </div>
