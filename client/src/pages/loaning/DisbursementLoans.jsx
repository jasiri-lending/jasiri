...
const DisbursedLoans = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch disbursed loans with enhanced branch and region data
      ...
      // Create regions map for quick lookup
      const regionsData = await supabase
        .from("regions")
  ...
  const getRegionName = (loan) => {
    // Try to get region from nested relationship first
    if (loan.customers?.branches?.regions?.name) {
      return loan.customers.branches.regions.name;
    }
    // Fallback to region map using region_id
    const regionId = loan.customers?.branches?.region_id;
    if (regionId && regionsMap[regionId]) {
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          <div className="text-xs text-brand-primary">
            <span className="font-semibold">{disbursedLoans.length}</span> disbursed loan{disbursedLoans.length !== 1 ? 's' : ''}
          </div>
        </div>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {currentLoans.map((loan, index) => {
                ...
                const regionName = getRegionName(loan);
