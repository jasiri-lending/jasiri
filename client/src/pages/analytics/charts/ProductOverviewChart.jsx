  ...
  return (
    <div className="bg-white/90 backdrop-blur-xl p-6 rounded-2xl shadow-2xl border border-white/40 min-w-[340px] relative z-[9999]">
      <div className="flex items-center gap-3 mb-4 border-b border-slate-100 pb-3">
        ...
        <div>
          <p className="text-xs  text-slate-600">Product Analysis</p>
          <p className=" text-slate-600 text-sm">{productData?.productName}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
          ...
          <p className="text-sm font-semibold text-slate-700">{productData?.totalCount}</p>
        </div>
        <div className="p-3 bg-emerald-50 rounded-xl border border-emerald-100">
          <p className="text-xs font-semibold text-emerald-400  mb-1">Total Amount</p>
          <p className="text-sm font-semibold text-emerald-700 ">Ksh {productData?.totalAmount?.toLocaleString()}</p>
        </div>
      </div>

      {productData?.subProducts && productData.subProducts.length > 0 && (
        <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          <p className="text-xs font-semibold text-slate-400  px-1">Product Types</p>
          {productData.subProducts.map((sub, idx) => (
            <div key={idx} className="p-3 bg-white rounded-xl border border-slate-50 shadow-sm hover:border-indigo-100 transition-colors">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-[10px] font-semibold text-brand-primary bg-indigo-50 px-2 py-0.5 rounded-full">{sub.percentage}%</span>
              </div>
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <span className="text-[8px] font-semibold text-slate-400 uppercase">Count</span>
                  <p className="text-xs font-semibold text-slate-600">{sub.count}</p>
                </div>
                <div className="text-right">
                  <span className="text-[8px] font-semibold text-slate-400 uppercase">Amount</span>
                  <p className="text-xs font-semibold text-emerald-600 tracking-tight">Ksh {sub.amount?.toLocaleString()}</p>
                </div>
              </div>
            </div>
          ))}
