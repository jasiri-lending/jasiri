...
const Verification = ({ customerId, onClose }) => {
    ...
    try {
      ...
      if (draftError) {
        ...
        setVerificationData((prev) => ({
          ...prev,
          customer: {
            idVerified: existingDraft.co_customer_id_verified ?? prev.customer.idVerified,
            phoneVerified: existingDraft.co_customer_phone_verified ?? prev.customer.phoneVerified,
            comment: existingDraft.co_customer_comment ?? prev.customer.comment,
          },
          business: {
            verified: existingDraft.co_business_verified ?? prev.business.verified,
            comment: existingDraft.co_business_comment ?? prev.business.comment,
          },
          loan: {
            ...prev.loan,
            scoredAmount: existingDraft.co_loan_scored_amount ?? prev.loan.scoredAmount,
            comment: existingDraft.co_loan_comment ?? prev.loan.comment,
          },
          guarantors: prev.guarantors.map((g, index) => ({
            ...g,
            idVerified: existingDraft.co_guarantor_id_verified ?? g.idVerified,
            phoneVerified: existingDraft.co_guarantor_phone_verified ?? g.phoneVerified,
            comment: existingDraft.co_guarantor_comment
              ? existingDraft.co_guarantor_comment.split("; ")[index] || ""
              : g.comment,
          })),
          security: {
            verified: existingDraft.co_borrower_security_verified ?? prev.security.verified,
            comment: existingDraft.co_borrower_security_comment ?? prev.security.comment,
          },
          guarantorSecurity: {
            verified: existingDraft.co_guarantor_security_verified ?? prev.guarantorSecurity.verified,
            comment: existingDraft.co_guarantor_security_comment ?? prev.guarantorSecurity.comment,
          },
          nextOfKin: {
            verified: existingDraft.co_next_of_kin_verified ?? prev.nextOfKin.verified,
            comment: existingDraft.co_next_of_kin_comment ?? prev.nextOfKin.comment,
          },
          document: {
            verified: existingDraft.co_document_verified ?? prev.document.verified,
            comment: existingDraft.co_document_comment ?? prev.document.comment,
          },
          overallComment: existingDraft.co_overall_comment ?? prev.overallComment,
          finalDecision: existingDraft.co_final_decision ?? prev.finalDecision,
        }));
  ...
  const submitVerification = async () => {
    try {
      ...
      // Check if draft exists and update it, or insert new record
